...

{
    ...
    g.drawRect(bounds);
    auto size = jmin(bounds.getWidth(), bounds.getHeight()) - JUCE_LIVE_CONSTANT(4);
    auto r = bounds.withSizeKeepingCentre(size, size).toFloat();

    float ang = 30.f;
    size -= JUCE_LIVE_CONSTANT(4);
...
ResponseCurveComponent::ResponseCurveComponent(EQ_PluginAudioProcessor& p) :
    audioProcessor(p),
   ...
   // auto responseArea = getLocalBounds();
    ...
    if (shouldShowFFTAnalysis)
    {
        ...
        leftChannelFFtPath.applyTransform(AffineTransform().
            translation(responseArea.getX(), responseArea.getY()));
     
        g.setColour(Colours::skyblue);
        g.strokePath(leftChannelFFtPath, PathStrokeType(1.f));

        auto rightChannelFFtPath = rightPathProducer.getPath();
        rightChannelFFtPath.applyTransform(AffineTransform().
            translation(responseArea.getX(), responseArea.getY()));
...
//==============================================================================
EQ_PluginAudioProcessorEditor::EQ_PluginAudioProcessorEditor (EQ_PluginAudioProcessor& p)
    ...
    auto safePtr = juce::Component::SafePointer<EQ_PluginAudioProcessorEditor>(this);
...
//==============================================================================
void EQ_PluginAudioProcessorEditor::paint (juce::Graphics& g)
{
    using namespace juce;
    // (Our component is opaque, so we must completely fill the background with a solid colour)
    g.fillAll (Colours::black);
...
void EQ_PluginAudioProcessorEditor::resized()
{
...
std::vector<juce::Component*> EQ_PluginAudioProcessorEditor::getComps()
{
